...
function otp($ure) {
    return hex2bin(random_bytes($ure));
}
$otp = otp($ure = 8);
...
if ($_SERVER['REQUEST_METHOD']) {
    ...
    $kagua = mysqli_query($conn, "SELECT * FROM `users` WHERE pepe = '$pepe'");
        ...
        if ($barua !== $pepe) {
            ...
            if (!$weka) {
                ...
                if (!$result) {
                    ...
                    header("Location: ../successful.php?succes=$succes&action=Sign in&url=signin.php");
                    exit;
                }
                else {
